...
function getPostLikeLink( $post_id ) {
	...
	if ( ( !$like_count ) || ( $like_count && $like_count == "0" ) ) { // no votes, set up empty variable
		...
		if ( AlreadyLiked( $post_id ) ) { // already liked, set up unlike addon
			$output .= '<span class="unliker"><i class="fa-duotone fa-times-circle"></i></span><span class="like prevliked"><i class="fa-duotone fa-heart"></i></span>';
			$output .= ' <span class="count alreadyliked">'.$likes.'</span></a></span>&nbsp; ';
		} else { // normal like button
			$output .= '<span class="unliker"></span><span class="like"><i class="fa-duotone fa-heart"></i></span>';
